...
interface SubCategoryListTableProps {
  ...
  onEditClick: (sc: SubCategory) => void;
  onDeleteClick: (id: number) => void;
}

export default function SubCategoryListTable({
  subCategories, onEditClick, onDeleteClick
}: SubCategoryListTableProps) {
      ...
      {subCategories.length === 0 ? (
        ...
        <div className="divide-y" style={{ borderColor: "var(--color-border)" }}>
          {subCategories.map((sc) => (
            <div key={sc.id} className="flex justify-between items-center py-4 group first:pt-0 last:pb-0">
              <div className="flex items-center gap-4 min-w-0">
                {sc.imageUrl ? (
                  <div className="h-11 w-14 bg-[var(--color-surface-alt)] border overflow-hidden flex-shrink-0 rounded-sm" style={{ borderColor: "var(--color-border)" }}>
                    <img src={resolveImageUrl(sc.imageUrl)} alt={sc.name} className="h-full w-full object-cover" />
                  </div>
                ) : (
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-mono text-[10px] text-[var(--color-brass)] mt-0.5 uppercase tracking-wider">
                    {sc.category?.name ?? "—"}
                  </p>
                  {sc.description && (
                    <p className="font-body text-[11px] text-[var(--color-ink-muted)] mt-0.5 truncate max-w-xs">{sc.description}</p>
                  )}
                  <p className="font-mono text-[10px] text-[var(--color-ink-faint)] mt-0.5 uppercase tracking-wider">ID: SUBCAT-{sc.id}</p>
                </div>
              </div>

              <div className="flex items-center gap-4 flex-shrink-0">
                <button type="button" onClick={() => onEditClick(sc)} className="text-[var(--color-ink-faint)] hover:text-[var(--color-green)] transition-colors p-1">
                  <Edit3 className="h-4 w-4" strokeWidth={1.5} />
                </button>
                <button type="button" onClick={() => onDeleteClick(sc.id)} className="text-[var(--color-ink-faint)] hover:text-red-600 transition-colors p-1">
